...
protected:
  ...
  void finalize_action() override {
    ...
    int r = librbd::cls_client::mirror_image_get(&get_io_ctx(), m_image_id, &mirror_image);
    if( r < 0) {
      std::cerr << "rbd: failed to execute cls_clent::mirror_image_get" << std::endl;
    }
    std::string image_type;
    if(mirror_image.type == cls::rbd::MIRROR_IMAGE_TYPE_STANDALONE){
      image_type = "standalone";
    }
    else if(mirror_image.type == cls::rbd::MIRROR_IMAGE_TYPE_GROUP){
      image_type = "group";
    }
    if (m_formatter != nullptr) {
      ...
      m_formatter->dump_string("type", image_type);
      if (local_site_r >= 0) {
...
int execute_status(const po::variables_map &vm,
  ...
  for (auto &[group_id, group_status] : mirror_groups) {
    ...
    if (!group_status.site_statuses.empty()) {
          for (auto& site : group_status.site_statuses) {
            mirror_groups_count[site.state]++;
          }
        }
  }
  ...
  if (formatter != nullptr) {
    ...
    std::cout<< "Groups: " << total_groups << " total" <<std::endl;
      for(auto &it1 : mirror_groups_count){
        std::cout << "    " << it1.second << " " 
      << utils::mirror_group_status_state(it1.first) <<std::endl;
      }
    }
  }
